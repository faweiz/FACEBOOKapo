...
//			uint16_t spi_rx = (m_rx_buf[0]<<8) | m_rx_buf[1];
	
			uint16_t spi_rx = ((m_rx_buf[0]&0x0F)<<8) | m_rx_buf[1];
	
			int ADC_value = spi_rx;
			double voltage_value = ADC_value / (4095/2.5);
//clear PrintBuffer buffer	
			memset(PrintBuffer,0x00,sizeof(PrintBuffer));
			sprintf((char*) PrintBuffer, "%.3f", voltage_value);
//get data length
