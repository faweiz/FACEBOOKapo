...
// SPI Variables		
		...
		int incomingValues[16];
//static uint8_t       m_tx_buf[]={0x18,0x00};
static uint8_t       m_tx_buf_ch[2];
static uint8_t       m_rx_buf[sizeof(m_tx_buf_ch) + 1];    /**< RX buffer. */
static const uint8_t m_length = sizeof(m_tx_buf_ch);        /**< Transfer length. */																						
static uint8_t       m_tx_buf_CH16[18][2] = { {0x18,0x00},
																							{0x10,0x80},
																							{0x11,0x00},
																							{0x11,0x80},
																							{0x12,0x00},
...
/* For faster display on phone		
				if(send_value == 0){
					send_incomingValues = incomingValues[0] / (4095/2.5);
...
void SPI_Function(uint8_t tx_buf[]){
	// Reset rx buffer and transfer done flag
        memset(m_rx_buf, 0x00, m_length);
        spi_xfer_done = false;

				APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, tx_buf, m_length, m_rx_buf, m_length));
		...
		while(1)
    {
		//****************** Modified on 12/20/2016 （UART + SPI_ADC) ******************//
			for(int i=0;i<18;i++)
			{
				for(int j=0; j<2;j++)
				{
					m_tx_buf_ch[j] = m_tx_buf_CH16[i][j];
				}
// Call SPI
				SPI_Function(m_tx_buf_ch);
				if(i>=2)
				{
// changed on 12/19/2016 10:51pm 				
				spi_rx = ((m_rx_buf[0]&0x0F)<<8) | m_rx_buf[1];
				ADC_value = spi_rx;
...
//					printf(" m_rx_buf[0] = %d \n",m_rx_buf[0]);
//					printf(" m_rx_buf[1] = %d \n",m_rx_buf[1]);
					incomingValues[i-2] = ADC_value;
				}
				nrf_delay_ms(30); //delay 30 msec
			}
			for(int k=0; k<16;k++)
			{
					if( send_value== k){
